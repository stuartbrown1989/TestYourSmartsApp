...
public class sportsQuizActivity extends AppCompatActivity {
                ...
                public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
                    ...
                    switch (menuItem.getItemId()) {
                        case R.id.nav_home:
                            ...
                            Intent intentHome = new Intent(sportsQuizActivity.this, MainActivity.class);
                        ...
                        case R.id.nav_leaderboard:
                            g_question_list.clear();
                            g_question_list.removeAll(Collections.emptyList());
                            Intent intentLeaderboard= new Intent(sportsQuizActivity.this, leaderboardChoiceActivity.class);
                            startActivity(intentLeaderboard);
                            return true;

                        case R.id.nav_account:
                            Intent intentAccount = new Intent(sportsQuizActivity.this, account_page.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Topic Quiz - Sports");
        ...
        timetoanswer = findViewById(R.id.Sports_timetoanswer);
        headerTimer = findViewById(R.id.SportstimeRemaining);
        theDifficulty = findViewById(R.id.Sports_difficulty);
        ...
        textViewQuestion = findViewById(R.id.Sportstext_view_question);
        textViewScore = findViewById(R.id.Sportstext_view_new_score);
        textViewQuestionCount = findViewById(R.id.Sportstext_view_question_count);
        textColorDefaultRb = op1.getTextColors();
        confirmNext = findViewById(R.id.Sportsbutton_confirm_next);
        timer = findViewById(R.id.Sportstext_view_timer);
        bottomNavigationView.setOnNavigationItemSelectedListener(onNavigationItemReselectedListener);
            ...
            public void onClick(View v) {
                ...
                if (!answered) {
                    if (op1.isChecked() || op2.isChecked() || op3.isChecked() || op4.isChecked()) {
                        ...
                        Toast.makeText(sportsQuizActivity.this, "Please select answer", Toast.LENGTH_SHORT).show();
                    }
                } else {
    ...
    private void getEasySportsQuestions()
    {
    ...
    private void getMediumSportsQuestions()
    {
    ...
    private void getHardSportsQuestions()
    {
    ...
    private void getuserScores()
    {
        ...
        {
            Intent resultintent = new Intent(sportsQuizActivity.this, resultPageActivity.class);
            resultintent.putExtra("SCORE", score);
            resultintent.putExtra("QUIZ", "Topic Quiz - Sports");
        ...
        if(Patterns.EMAIL_ADDRESS.matcher(userEmail).matches()) {
            ...
            DocumentReference userRef = g_firestore.collection("Leaderboards").document("Topic").collection("Sports").document(userEmail);
            userRef.get()
                        ...
                        public void onSuccess(DocumentSnapshot snapshot) {
                            ...
                            if (snapshot.exists()) {
                                ...
                                if (checkscore < score) {
                                    ...
                                    gkscore.put("QuizType", "Sports");
                                    g_firestore.collection("Leaderboards").document("Topic").collection("Sports").document(userEmail).set(gkscore);
                                    setUserStatistics();
                                    startActivity(resultintent);
                                    g_question_list.clear();
                                    questionIDs.removeAll(Collections.emptyList());
                    ...
                    .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                        ...
                        public void onSuccess(DocumentSnapshot snapshot) {
                            ...
                            if (snapshot.exists()) {
                                ...
                                checkstoredTopicAverage = snapshot.getDouble("Sports Average");
                                ...
                                {
                                    g_firestore.collection("Statistics").document("Easy").collection("Users").document(userEmail).update("Sports Average", time);
                                }
                                ...
                                if(checkstoredTopicAverage != null)
                                {
                                    ...
                                    g_firestore.collection("Statistics").document("Easy").collection("Users").document(userEmail).update("Sports Average", calcNewTopicAverage);
                                }
                            ...
                            if (!snapshot.exists()) {
                                ...
                                gkstats.put("Sports Average", time);
                    ...
                    .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                        ...
                        public void onSuccess(DocumentSnapshot snapshot) {
                            ...
                            if (snapshot.exists()) {
                                ...
                                if(checkstoredTopicAverage == null)
                                {
                                    g_firestore.collection("Statistics").document("Medium").collection("Users").document(userEmail).update("Sports Average", time);
                                }
                                //If there is a Topic Average Score stored, then calculate the new average
                                if(checkstoredTopicAverage != null)
                                {
                                    calcNewTopicAverage = (time + checkstoredTopicAverage) / 2;
                                    g_firestore.collection("Statistics").document("Medium").collection("Users").document(userEmail).update("Sports Average", calcNewTopicAverage);
                                }
                                //Always calculate the overall average  - This field will always be here if the snapshot exists, so no need for conditional null statement
                                calcNewAverage = (time + checkstoredaverage) / 2;
                    ...
                    .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                        ...
                        public void onSuccess(DocumentSnapshot snapshot) {
                            ...
                            if (snapshot.exists()) {
                                ...
                                if(checkstoredTopicAverage == null)
                                {
                                    g_firestore.collection("Statistics").document("Hard").collection("Users").document(userEmail).update("Sports Average", time);
                                }
                                //If there is a Topic Average Score stored, then calculate the new average
                                if(checkstoredTopicAverage != null)
                                {
                                    calcNewTopicAverage = (time + checkstoredTopicAverage) / 2;
                                    g_firestore.collection("Statistics").document("Hard").collection("Users").document(userEmail).update("Sports Average", calcNewTopicAverage);
                                }
                                //Always calculate the overall average  - This field will always be here if the snapshot exists, so no need for conditional null statement
                                calcNewAverage = (time + checkstoredaverage) / 2;
