...
public class geographyQuizActivity extends AppCompatActivity {
    ...
    private String userName, checkUser, checkTopic;
    private String currentTopic = "Geography";
                ...
                public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
                    ...
                    switch (menuItem.getItemId()) {
                        case R.id.nav_home:
                            ...
                            Intent intentHome = new Intent(geographyQuizActivity.this, MainActivity.class);
                        ...
                        case R.id.nav_leaderboard:
                            g_question_list.clear();
                            g_question_list.removeAll(Collections.emptyList());
                            Intent intentLeaderboard= new Intent(geographyQuizActivity.this, leaderboardChoiceActivity.class);
                            startActivity(intentLeaderboard);
                            return true;

                        case R.id.nav_account:
                            Intent intentAccount = new Intent(geographyQuizActivity.this, account_page.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Topic Quiz - Geography");
        bottomNavigationView = findViewById(R.id.top_nav_bar);
        main_frame = findViewById(R.id.main_frame);

        final AlphaAnimation buttonClick = new AlphaAnimation(1F, 0.8F);
        //Initializing Variables
        timetoanswer = findViewById(R.id.GEO_timetoanswer);
        headerTimer = findViewById(R.id.GEOtimeRemaining);
        theDifficulty = findViewById(R.id.GEO_difficulty);
        ...
        textViewQuestion = findViewById(R.id.GEOtext_view_question);
        textViewScore = findViewById(R.id.GEOtext_view_new_score);
        textViewQuestionCount = findViewById(R.id.GEOtext_view_question_count);
        textColorDefaultRb = op1.getTextColors();
        confirmNext = findViewById(R.id.GEObutton_confirm_next);
        timer = findViewById(R.id.GEOtext_view_timer);
            ...
            public void onClick(View v) {
                ...
                if (!answered) {
                    if (op1.isChecked() || op2.isChecked() || op3.isChecked() || op4.isChecked()) {
                        ...
                        Toast.makeText(geographyQuizActivity.this, "Please select answer", Toast.LENGTH_SHORT).show();
                    }
                } else {
    ...
    //Get all Easy Geography questions from firestore
...
//            questionIDs.add(currentQuestion.getID());
    ...
    private void getuserScores()
    {
        ...
        {
            Intent resultintent = new Intent(geographyQuizActivity.this, resultPageActivity.class);
            resultintent.putExtra("SCORE", score);
            resultintent.putExtra("QUIZ", "Topic Quiz - Geography");
        ...
        if(Patterns.EMAIL_ADDRESS.matcher(userEmail).matches()) {
            ...
            DocumentReference userRef = g_firestore.collection("Leaderboards").document("Topic").collection("Geography").document(userEmail);
            userRef.get()
                        ...
                        public void onSuccess(DocumentSnapshot snapshot) {
                            ...
                            if (snapshot.exists()) {
                                ...
                                if (checkscore < score) {
                                    ...
                                    gkscore.put("QuizType", "Geography");
                                    g_firestore.collection("Leaderboards").document("Topic").collection("Geography").document(userEmail).set(gkscore);
                                    setUserStatistics();
                                    startActivity(resultintent);
                                    g_question_list.clear();
                                    questionIDs.removeAll(Collections.emptyList());
                        ...
                        .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                            ...
                            public void onSuccess(DocumentSnapshot snapshot) {
                                ...
                                if (snapshot.exists()) {
                                    ...
                                    checkstoredTopicAverage = snapshot.getDouble("Geography Average");
                                    ...
                                    if (checkstoredTopicAverage == null) {
                                        g_firestore.collection("Statistics").document("Easy").collection("Users").document(userEmail).update("Geography Average", time);
                                    }
                                    ...
                                    if (checkstoredTopicAverage != null) {
                                        ...
                                        g_firestore.collection("Statistics").document("Easy").collection("Users").document(userEmail).update("Geography Average", calcNewTopicAverage);
                                    }
                                ...
                                if (!snapshot.exists()) {
                                    ...
                                    gkstats.put("Geography Average", time);
                        ...
                        .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                            ...
                            public void onSuccess(DocumentSnapshot snapshot) {
                                ...
                                if (snapshot.exists()) {
                                    ...
                                    //If there is no Topic Average stored, just add the time for this quiz only to a new field for the Selected Topic
                                    if (checkstoredTopicAverage == null) {
                                        g_firestore.collection("Statistics").document("Medium").collection("Users").document(userEmail).update("Geography Average", time);
                                    }
                                    //If there is a Topic Average Score stored, then calculate the new average
                                    if (checkstoredTopicAverage != null) {
                                        calcNewTopicAverage = (time + checkstoredTopicAverage) / 2;
                                        g_firestore.collection("Statistics").document("Medium").collection("Users").document(userEmail).update("Geography Average", calcNewTopicAverage);
                                    }
                                    //Always calculate the overall average  - This field will always be here if the snapshot exists, so no need for conditional null statement
                                    calcNewAverage = (time + checkstoredaverage) / 2;
                        ...
                        .addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                            ...
                            public void onSuccess(DocumentSnapshot snapshot) {
                                ...
                                if (snapshot.exists()) {
                                    ...
                                    //If there is no Topic Average stored, just add the time for this quiz only to a new field for the Selected Topic
                                    if (checkstoredTopicAverage == null) {
                                        g_firestore.collection("Statistics").document("Hard").collection("Users").document(userEmail).update("Geography Average", time);
                                    }
                                    //If there is a Topic Average Score stored, then calculate the new average
                                    if (checkstoredTopicAverage != null) {
                                        calcNewTopicAverage = (time + checkstoredTopicAverage) / 2;
                                        g_firestore.collection("Statistics").document("Hard").collection("Users").document(userEmail).update("Geography Average", calcNewTopicAverage);
                                    }
                                    //Always calculate the overall average  - This field will always be here if the snapshot exists, so no need for conditional null statement
                                    calcNewAverage = (time + checkstoredaverage) / 2;
